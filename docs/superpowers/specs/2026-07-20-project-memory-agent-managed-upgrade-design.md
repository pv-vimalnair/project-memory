...
- the coordinator's standard integration audit, produced for every canonical mutation.
...
7. Exactly one standard coordinator integration audit binds the finalized plan and derived-view hashes; fresh startup returns `resume` with the five-file reading order.
